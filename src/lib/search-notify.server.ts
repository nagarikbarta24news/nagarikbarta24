...
async function submitSitemapToGoogle(): Promise<{ ok: boolean; status?: number; skipped?: boolean; error?: string }> {
  const now = Date.now();
  ...
  if (!lovableKey || !gscKey) return { ok: false, error: "gsc-not-configured" };

  try {
    const res = await fetch(
      `${GSC_GATEWAY}/webmasters/v3/sites/${encodeURIComponent(SITE_ORIGIN + "/")}/sitemaps/${encodeURIComponent(SITEMAP_URL)}`,
      {
        method: "PUT",
        headers: {
